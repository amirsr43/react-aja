...
void main() {
  ...
  float minDim = min(uCanvas.x, uCanvas.y);
  vec2 q = rp * (uCanvas / minDim);
  q /= max(uScale, 0.0001);
  q /= 0.5 + 0.2 * dot(q, q);
  q += 0.2 * cos(t) - 7.56;
...
export function ColorBends({
  ...
  const scaledBandWidth = bandWidth <= 1.0 ? bandWidth * 40.0 : bandWidth;
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBandWidth: { value: scaledBandWidth }
      },
  ...
  useEffect(() => {
    ...
    material.uniforms.uBandWidth.value = scaledBandWidth;
    ...
    const finalColors = colors && colors.length > 0 ? colors : (color ? [color] : []);
