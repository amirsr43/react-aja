// src/components/docs/InfiniteImageMarqueeCustomizer.jsx
import React, { useState } from "react";
import { Copy, Check } from "lucide-react";
import InfiniteImageMarquee from "../ui/animations/InfiniteImageMarquee";
...
export default function InfiniteImageMarqueeCustomizer() {
  ...
  const [cardHeight,    setCardHeight]    = useState("180px");
  const [numRows,       setNumRows]       = useState(2);

  const currentDoc = docsData["infinite-image-marquee"];
          ...
          <div className="tab-preview-pane">
            ...
            <div style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "20px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  <label className="customizer-label">
                    Card Height — <span style={{ color: "#a78bfa" }}>{cardHeight}</span>
                  </label>
                  <input
                    type="range" min={100} max={280} step={10}
                    value={parseInt(cardHeight)}
                    onChange={(e) => setCardHeight(`${e.target.value}px`)}
                    style={{ accentColor: "#a78bfa", width: "100%", marginTop: "6px" }}
                  />
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  ...
                  <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "4px" }}>
                    {["0.5rem", "1rem", "1.5rem", "2rem", "2.5rem"].map((g) => (
                      <button
                        key={g}
                        onClick={() => setGap(g)}
                        style={{
